...
- `searchIndex=KindleStore`はCreators API側で機能していない(物理商品が混入する)ため、`scripts/fetch_sales.py`側で`itemInfo.classifications.productGroup`に`"Ebook"`を含む商品だけに絞り込んでいます。`browse_node_id`を指定しないジャンルはこのフィルタだけが頼りになるため、母数が少なくなりがちです
